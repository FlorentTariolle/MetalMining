...
from analysis.process_wordcloud_metalness import load_music_data_with_lyrics,process_metal_songs, process_non_metal_songs
import pandas as pd
...
def process_idf_scores(metal_df, non_metal_df, output = None):
    ...
    if output:
        if os.path.exists(output):
                os.remove(output)
        os.makedirs(os.path.join(_get_project_root(), "output_data"), exist_ok=True)
